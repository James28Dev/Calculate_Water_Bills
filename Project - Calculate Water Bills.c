...
struct provincialWaterworksAuthority {
	...
	int meterSize;
	float consump;
	struct stringProvincialWaterworksAuthority str;
} dataWater;
...
int main() {
	...
	do {	
		title("\t\t\t\t       MAIN MENU CALCULATION PROGRAM ", 104, '=');
		...
		if(menuMain == 1) {
			do{
				title("\t\t  PROGRAM FOR CALCULATING WATER BILLS PROVINCIAL WATERWORKS AUTHORITY", 104, '=');
				outputChoiceMenu(&dataWater);
				line(72,'-');
				...
				if(menuSub == 5) {			
					...
					if(dataWater.waterUsed == 1) {
						if(dataWater.userType == 1) {
							...
							calculate(dataWater.consump, &rates, dataWater.meterSize);
						} else 
						if(dataWater.userType == 2) {
				...
				if(menuSub == 6) {
					do{
						...
						title("\t\t\t\t      WATER TARIFF GRAPH PROVINCIAL WATERWORKS AUTHORITY", 124, '=');
						printf("\n\n\tWater tariff graph\n\n");
						printf("\t 1.  General area (Except for special areas)\n");
						printf("\t 2.  Branch areas in Phuket, Koh Samui and Koh Phangan\n");
						printf("\t 3.  Private investment space\n");
						...
						if(graph == 1) {
							...
							outputGraph(housing, small, large, graph);				
						} else 
						if(graph == 2) {
							float housing[][4] = {{10.20, 16.00, 19.00, 21.20}, {102.00, 160.00, 190.00, 424.00}, {102.00, 262.00, 452.00, 876.00}};
